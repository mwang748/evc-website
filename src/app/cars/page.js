    ...
    useEffect(() => {
        ...
        const mainTl = gsap.timeline({
            scrollTrigger: {
                ...
                onUpdate: (self) => {
                    // Use ScrollTrigger's progress (0 to 1) directly for accurate tracking
                    const progress = self.progress;

                    // Calculate continuous position across all years
                    const continuousIndex = progress * (yearArray.length - 1);
                    const currentYearIndex = Math.min(
                        Math.round(continuousIndex),
                        yearArray.length - 1
                    );

                    // Update year dots with proximity-based scaling
                    yearDots.forEach((dot, index) => {
                        // Calculate this year's position (0 to 1)
                        const yearPosition = index / (yearArray.length - 1);

                        // Calculate distance from current progress
                        const distance = Math.abs(progress - yearPosition);

                        // Calculate threshold (half a section)
                        const threshold = 0.5 / (yearArray.length - 1);

                        if (distance < threshold) {
                            // Within range - calculate proximity for smooth scaling
                            const proximity = 1 - (distance / threshold);
                            const scale = 1 + (proximity * 0.2); // Scale from 1.0 to 1.2

                            dot.style.transform = `scale(${scale})`;

                            // Mark as active if this is the closest year
                            ...
                            // Outside range - reset to normal size
                            dot.classList.remove('active');
